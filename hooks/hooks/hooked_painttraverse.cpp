...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (c_config::get()->b["skins_maskchanger"])
	{
	...
	if (reload_fonts())
	{
		...
		fonts[ESP] = create_font(crypt_str("SmallFont"), 8 * CMenu::get()->GetDPINum(), FW_MEDIUM, FONTFLAG_OUTLINE); //Smallest Pixel-7
		fonts[LMAO] = create_font(crypt_str("Verdana"), 26 * CMenu::get()->GetDPINum(), FW_MEDIUM, FONTFLAG_OUTLINE); //Smallest Pixel-7
		fonts[NAME] = create_font(crypt_str("Lucida console"), 12 * CMenu::get()->GetDPINum(), FW_EXTRALIGHT, FONTFLAG_ANTIALIAS);
		fonts[HE] = create_font(crypt_str("Verdana"), 12 * CMenu::get()->GetDPINum(), FW_MEDIUM, FONTFLAG_ANTIALIAS);
		...
		fonts[GRENADES] = create_font(crypt_str("undefeated"), 30 * CMenu::get()->GetDPINum(), FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[INDICATORFONT] = create_font(crypt_str("Calibri"), 34 * CMenu::get()->GetDPINum(), 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[DAMAGE_MARKER] = create_font(crypt_str("Verdana"), 16 * CMenu::get()->GetDPINum(), FW_HEAVY, FONTFLAG_OUTLINE);
		fonts[VELOCITY] = create_font(crypt_str("Verdana"), 35 * CMenu::get()->GetDPINum(), FW_HEAVY, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[INDICATORFONT2] = create_font(crypt_str("Calibri"), 34 * CMenu::get()->GetDPINum(), FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	...
	if (panel_id == panel)
	{
		if (g_ctx.available())
		{
			...
			if (c_config::get()->auto_check(c_config::get()->i["esp_en"], c_config::get()->i["esp_en_type"]) && g_cfg.esp.damage_marker)
