...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	if (stage == FRAME_RENDER_END)
	{
		...
		//c_config::get()->auto_check(c_config::get()->i["esp_en"], c_config::get()->i["esp_en_type"]) && g_cfg.esp.removals[REMOVALS_ZOOM] && g_cfg.esp.fix_zoom_sensivity && 
		if (zoom_sensitivity_ratio_mouse->GetFloat() == 1.0f) //-V550
			zoom_sensitivity_ratio_mouse->SetValue(0.0f);
	    //(!c_config::get()->auto_check(c_config::get()->i["esp_en"], c_config::get()->i["esp_en_type"]) || !g_cfg.esp.removals[REMOVALS_ZOOM] || !g_cfg.esp.fix_zoom_sensivity) && 
		else if (!zoom_sensitivity_ratio_mouse->GetFloat())
			zoom_sensitivity_ratio_mouse->SetValue(1.0f);
		//axar hhemizlia es uketesobistvisaa ;D:D
