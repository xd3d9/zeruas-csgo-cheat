...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	render::get().circle(screen_origin.x, screen_origin.y - size.y * 0.5f, 85, 28, Color(0, 0, 0));

	Vector mins, maxs;
	inferno->GetClientRenderable()->GetRenderBounds(mins, maxs);

	// render::get().Draw3DFilledCircle(origin, 120.0f, g_cfg.esp.molotov_timer_color);
	// render::get().Draw3DFilledCircle(origin, Vector(maxs - mins).Length2D() * 0.5, g_cfg.esp.molotov_timer_color);

	render::get().CircularProgressBar(screen_origin.x, screen_origin.y - size.y * 0.8f, 28, -20, (360 * factor), 2.5f, Color(255, 255, 255, 144));

	render::get().text(fonts[LMAO], screen_origin.x + 1.0f, screen_origin.y - size.y * 0.5f - 6.0f, Color(255, 255, 0), HFONT_CENTERED_X | HFONT_CENTERED_Y, "!");
