...
void c_config::import() {
	...
	if (is_number(std::to_string(c_config::get()->i["_preset"])) == false) {
		...
		std::string config_name = Config_input_text + std::string(".ini");
...
void c_config::save() {
	...
	sprintf(file_path, "C:/gs/cfg/%s.ini", (std::string(Config_input_text)).c_str());
...
void c_config::load_keys() {
	for (int k = 0; k < presets.size(); k++) {
		...
		i[buffer] = GetPrivateProfileIntA("k", buffer, 0, "C:/gs/cfg/csgo_keys.ini");
	}
}
...
void c_config::save_keys() {
	for (int k = 0; k < presets.size(); k++) {
		...
		WritePrivateProfileStringA("k", buffer, value, "C:/gs/cfg/csgo_keys.ini");
	}
}