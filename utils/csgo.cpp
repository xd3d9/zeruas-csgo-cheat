...
CGlobalVarsBase* C_CSGO::m_GlobalVars() {
	if (!p_GlobalVars)
		p_GlobalVars = **reinterpret_cast<CGlobalVarsBase***>((*(DWORD**)(m_client()))[11] + 0xA);
...
{
	if (!p_weaponsys)
		p_weaponsys = *reinterpret_cast<IWeaponSystem**>(util::FindSignature(crypt_str("client.dll"), crypt_str("8B 35 ? ? ? ? FF 10 0F B7 C0")) + 2);
