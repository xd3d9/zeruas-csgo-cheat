...
void CMenu::Draw() {
	...
	if (this->m_nCurrentTab == 1) {
		ui::BeginChild("Anti-aimbot angle", { Vec2(0, 0), Vec2(3, 10) });
			...
			if (aatab == true)
			{
				...
				ui::SingelSelect("Yaw jitter", &cfg->i["aa_yawjit"], { "Off", "Offset", "Center" });
				if (cfg->i["aa_yawjit"])
				...
				ui::SingelSelect("Freestanding", &cfg->i["aa_fs"], { "Freestanding", "Default" });
				ui::Keybind("feestand", &cfg->i["freestand_key"], &cfg->i["freestand_key_style"]);
				ui::SliderInt("Roll", &cfg->i["rollaa_value"], -50, 50);
			}
	...
	if (this->m_nCurrentTab == 2) {
		ui::BeginChild("Weapon type", { Vec2(0, 0), Vec2(9, 0) }, Flags_NoResize | Flags_NoMove);
	...
	if (this->m_nCurrentTab == 3) {
		...
		ui::BeginChild("Other ESP", { Vec2(6, 0), Vec2(3, 3) });
			...
			ui::MultiSelectOld("Feature indicators", &cfg->m["ftind"], { "Force safe point", "Force body aim", "Ping spike", "Double tap", "Duck peek assist", "Freestanding" });
	...
	if (this->m_nCurrentTab == 4) {
		...
		if (cfg->b["air_starfe"])
		{
			ui::SingelSelect("Air strafe mode", &cfg->i["strafermode"], { "Viewangles", "Movement keys" });
	...
	if (this->m_nCurrentTab == 5) {
		...
		if (cfg->b["skins_knifechanger"])
			(ui::SingelSelectNoText("Knife", &cfg->i["skins_knifechanger_model"],
				{
				"Default",
				"M9 Bayonet",
				"Bayonet",
		...
		if (cfg->b["skins_glovechanger"])
			(ui::SingelSelectNoText("Glove", &cfg->i["skin_changer_model_glove"],
				{
				"Default",
				"Dallas",
				"Battle Mask",
		...
		if (cfg->b["skins_agentchanger"])
			(ui::SingelSelectNoText("Agent", &cfg->i["skin_changer_model_agent"], {
	"None",
	...
	"Dragomir | Sabre Footsoldier",
		...
		ui::InputText("_", search, 0, 0);
		static char lnameer2[999999] = "\0";
		ui::ListBox("wepsel", Vec2(163, 305), true, Flags_Listbox);
	...
	if (this->m_nCurrentTab == 7) {
		...
		if (oldscriptid != scriptIDD) {
			GetPrivateProfileStringA(crypt_str("autoload"), crypt_str(rand.c_str()), "", szStartupval, 64, crypt_str("C:\\gs\\luas\\menu.ini"));
